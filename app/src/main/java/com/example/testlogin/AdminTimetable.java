    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addTimetable.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (validate()) {
                    _daySchedule.add(new TimetableClass(_subjectName, _subjectTiming, _subjectTeacher));
                    DatabaseReference NODE = FirebaseDatabase.getInstance().getReference().child("School").child("TimetableRecord").child(TTclass.getSelectedItem().toString()).child(TTsection.getSelectedItem().toString()).child(TTday.getSelectedItem().toString()).child(TTsubjectName.getSelectedItem().toString());
                    NODE.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            if (!snapshot.exists()) {
                                NODE.setValue(_daySchedule.get(0)).addOnSuccessListener(new OnSuccessListener<Void>() {
                                    @Override
                                ...
                                Toast.makeText(AdminTimetable.this, "Timetable For The Day Already Exists", Toast.LENGTH_SHORT).show();
                        }
